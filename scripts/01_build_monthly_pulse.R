...
if ("five_cities" %in% allowed_domains || "province" %in% allowed_domains) {
  .epm_abort("Monthly pulse cannot use `five_cities` or `province` domains.")
}

epm_validate_layer_domains(layer, survey_type, allowed_domains, config)
output_paths <- epm_prepare_layer_outputs(layer, config)

message("Monthly pulse builder scaffold complete.")
message(sprintf("Resolved monthly period: %s", period))
message(sprintf("Validated monthly domains: %s", paste(allowed_domains, collapse = ", ")))
message("Prepared monthly output routes:")
for (output_name in names(output_paths)) {
  message(sprintf("- %s: %s", output_name, output_paths[[output_name]]))
}
message("Real monthly indicator calculation is pending by design; no analytical output was written.")
